...
Block.prototype.$setConfig = function(configUpdate) {
  for (var key in configUpdate) {
    if (!_.isEqual(this.config[key], configUpdate[key])) {
      // NOTE: Dependent changes are inconvenient now, as the config obj is only partially
      // updated when the event is triggered. Instead should perhaps gather changes and emit
      // them after config update, or listen once to 'configUpdated' event, to avoid race conditions.
...
Block.prototype.render = function() {
  ...
  var params = _.extend({}, this.config, {username: siteConfig.username}, dictionary); // Copies many keys, could use a dict prop too.
